        ...
        <nav className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            <div className="relative" onMouseEnter={handleMenuEnter} onMouseLeave={handleMenuLeave}>
              <Link href={`/${i18n.language}/generator`} className="text-gray-700 hover:text-purple-600 font-medium px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-purple-300 transition" type="button" onClick={() => setMenuOpen(false)}>{t('generator')}</Link>
              {menuOpen && (
                <div className="absolute left-0 mt-2 bg-white shadow-lg rounded-lg min-w-[180px] z-50 border border-gray-100 animate-fade-in">
                  {categories.map(cat => (
                    <Link key={cat.slug} href={`/${i18n.language}/generator/${cat.slug}`} className="flex items-center gap-2 px-4 py-2 text-purple-600 hover:bg-purple-50 hover:text-pink-500 transition rounded">
